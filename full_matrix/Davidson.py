...
def Davidson(matrix, n_eig, tolerance = 1e-8):
    ...
    for i in range(rows // 2):
        ...
        u = guess_space @ eigenvectors[:, :n_eig]
        ...
        if np.allclose(residuals, 0, atol = tolerance):
            ...
            message = "The dimension of the Davidson system is: " + str(len(eigenvalues)) + " and the eigenvalues are: " + str(eigenvalues[:n_eig])
            print(message)
            break
        # if the norm of the residuals is not lower than our tolerance, we need to add a new guess vector to our guess space
        else:
            ...
            for j in range(n_eig):
                ...
                if np.abs(eigenvalues[j] - matrix[j,j]) < tolerance:
                    ...
                    new_direction[:, j] = residuals[j] / (eigenvalues[0] - matrix[j,j])
