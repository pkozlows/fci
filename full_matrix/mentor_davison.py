...
def generalized_davidson_diagonalization(matrix, n_eigenvalues, search_dim_multiplier = 2,
    ...
    for iter in range(max_iter):
        ...
        residuals = np.dot(weight_matrix, np.diagflat(guess_eigenvalues[:n_eigenvalues])) - \
                    np.dot(ritz_vectors, guess_eigenvectors[:, :n_eigenvalues])
...
print(generalized_davidson_diagonalization(hamiltonian, 6))